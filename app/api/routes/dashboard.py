...
async def add_beneficiary(payload: AddBeneficiaryRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    u = await _get_user(db, user.id)
    if not u.is_pin_set or not u.auth_pin_hash:
    ...
    if total_pct + payload.percentage_share > 100:
        raise HTTPException(status_code=400, detail=f"Total cannot exceed 100%. Currently allocated: {total_pct}%")

    bank_code = await paystack_service.find_bank_code(payload.bank_name)
    if not bank_code:
        raise HTTPException(status_code=400, detail=f"Bank '{payload.bank_name}' not recognised")

    try:
        resolved = await paystack_service.resolve_account(payload.account_number, bank_code)
    except paystack_service.PaystackError as e:
        raise HTTPException(status_code=400, detail=f"Account verification failed: {e.message}")

    verified_name = resolved.get("account_name", payload.full_name)

    try:
        recipient_data = await paystack_service.create_transfer_recipient(
            name=verified_name, account_number=payload.account_number, bank_code=bank_code)
        recipient_code = recipient_data.get("recipient_code")
    except paystack_service.PaystackError as e:
        raise HTTPException(status_code=400, detail=f"Could not register beneficiary: {e.message}")

    bene = Beneficiary(
        user_id=u.id, full_name=verified_name, bank_name=payload.bank_name,
        bank_code=bank_code, account_number=payload.account_number,
        percentage_share=payload.percentage_share, paystack_recipient_code=recipient_code,
        is_verified=True,
    )
    db.add(bene)
    await db.flush()
    return {
        ...
        "beneficiary": {
            "id": bene.id, "full_name": bene.full_name, "verified_name": verified_name,
            "bank_name": bene.bank_name, "account_number": bene.account_number,
...
async def do_checkin(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    checkin.next_due_date = now + timedelta(days=checkin.checkin_interval_days)
...
async def update_checkin_config(payload: CheckinConfigRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if not checkin:
        raise HTTPException(status_code=404, detail="Check-in record not found")
    checkin.checkin_interval_days = payload.checkin_interval_days
    checkin.grace_period_days = payload.grace_period_days
    checkin.next_due_date = checkin.last_checkin_date + timedelta(days=payload.checkin_interval_days)
    await db.flush()
