# """app/services/isw_service.py

# Interswitch service layer for The Nexus.
# Covers:
#   - Inline Checkout payment initiation (client-side widget)
#   - Server-side transaction requery (verify payment after callback / webhook)
#   - OAuth2 token acquisition for server-to-server calls
#   - Funds transfer (disbursement to beneficiary bank accounts)

# Environment variables used (all in config.py / .env):
#     ISW_MERCHANT_CODE, ISW_PAY_ITEM_ID, ISW_MODE,
#     ISW_REQUERY_BASE_URL, ISW_WEBHOOK_SECRET,
#     ISW_CLIENT_ID, ISW_CLIENT_SECRET,
#     ISW_TERMINAL_ID, ISW_INITIATING_ENTITY_CODE
# """
...
Covers:
  ...
  - OAuth2 token acquisition for server-to-server calls
  - Funds transfer (disbursement to beneficiary bank accounts)
  - Bank list fetch + fuzzy name matching (Marketplace Identity API)
  - Account number resolution / name enquiry (Marketplace Identity API)
...
Environment variables used (all in config.py / .env):
    ...
    ISW_CLIENT_ID, ISW_CLIENT_SECRET,
    ISW_TERMINAL_ID, ISW_INITIATING_ENTITY_CODE,
    ISW_MARKETPLACE_BASE_URL  (defaults to https://api-marketplace-routing.k8.isw.la)
"""
from __future__ import annotations
