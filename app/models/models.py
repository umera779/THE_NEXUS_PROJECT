...
class Transaction(Base):
    ...
    # ISW-specific fields — added for Interswitch Inline Checkout integration
    txn_ref: Mapped[str | None] = mapped_column(String(100), unique=True, nullable=True, index=True)
    amount_kobo: Mapped[int | None] = mapped_column(Integer, nullable=True)
    payment_reference: Mapped[str | None] = mapped_column(String(150), nullable=True)   # ISW's own PaymentReference
    confirmed_via: Mapped[str | None] = mapped_column(String(20), nullable=True)        # "callback" | "webhook"
...
class Checkin(Base):
    ...
    # Stored as total seconds so intervals can be set down to the second (DD:HH:MM:SS)
    checkin_interval_seconds: Mapped[int] = mapped_column(Integer, nullable=False, default=15552000)  
    grace_period_seconds: Mapped[int] = mapped_column(Integer, nullable=False, default=2592000)       
    next_due_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    status: Mapped[str] = mapped_column(String(20), default=CheckinStatus.PENDING)
    ...
    # ── Convenience properties that express the interval/grace as fractional days
    # (kept for any legacy code that still reads .checkin_interval_days / .grace_period_days)
    @property
    def checkin_interval_days(self) -> float:
        return self.checkin_interval_seconds / 86400

    @property
    def grace_period_days(self) -> float:
        return self.grace_period_seconds / 86400